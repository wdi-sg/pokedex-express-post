...
app.get('/pokemon/new', (request, response) => {
    ...
    let html = `<html><head><title>New Pokemon</title></head><body><form method="POST" action="/pokemon">Enter a new pokemon<input type="hidden" name="id"><input type="hidden" name="num"><input type="text" name="name" placeholder="Enter Name"><input type="text" name="img" placeholder="Enter an image URL"><input type="text" name="height" placeholder="Enter a height"><input type="text" name="weight" placeholder="Enter a weight"><input type="submit" value="Submit"></form></body></html>`;
    response.send(html);
...
app.post('/pokemon', (request, response) => {
    ...
    jsonfile.readFile(FILE, (err, obj) => {
        ...
        let idOfLastPokemon = parseInt(obj['pokemon'][allPokemon.length - 1]['id']);
        let numOfLastPokemon = parseInt(obj['pokemon'][allPokemon.length - 1]['num']);
        let newPokemon = request.body;
        newPokemon['id'] = idOfLastPokemon + 1;
        newPokemon['num'] = numOfLastPokemon + 1;
        newPokemon['height'] += ` m`;
...
app.get('/search', (request, response) => {
    ...
    jsonfile.readFile(FILE, (err, obj) => {
        ...
        if (request.query.sortby === 'name') {
            ...
            let uniqueArray = sortedArray.filter((element) => {
