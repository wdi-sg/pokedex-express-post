...
class Main extends React.Component {
  ...
  render() {
    ...
    //map all pokemon objects into nice boxes
    const pokTable = pokemon.map(pokObj=>{
        let pokUrl = "/pokemon/"+pokObj.id;
        return <div style={{width:220}}>
                    <div style={{backgroundColor:"#f2f2f2",position:"relative",height:210,borderRadius:10}}>
                        <a href={pokUrl}>
                            <img src={pokObj.img} style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:200}}/>
                        </a>
                    </div>
                    <div>
                        <p style={{fontSize:12,color:"grey",fontWeight:"bold"}}>#{pokObj.num}</p>
                        <p style={{fontWeight:"bold"}}>{pokObj.name}</p>
                    </div>
                </div>
    })
    ...
    return (
      <html>
        <body style={{width:1000,margin:"0 auto",fontFamily:"sans-serif",textAlign:"center"}}>
            ...
            <form>
                {sortingMethodSelections}<br/><br/>
                <input type="submit" value="Submit"/>
            </form>
            <br/>
            <a href="/pokemon/new" style={{display:"block",textAlign:"left"}}>Add New Pokemon</a>
            <br/>
